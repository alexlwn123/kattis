...
//    vector<int> row;
//    for (int c = 0; c < 9; c++) {
//      if (grid[r][c] != '.') row.push_back(grid[r][c] - '0');
//    }
//    
//    set<int> s(row.begin(), row.end());
//    if (row.size() != s.size()) return false;
//  }
//
//  return true;
//}
...
//    vector<int> column;
//    for (int c = 0; c < 9; c++) {
//      if (grid[c][r] != '.') column.push_back(grid[r][c]-'0');
//    }
//    set<int> s(column.begin(), column.end());
...
//        for (int dc = 0; dc < 3; dc++) {
//          char cell = grid[r+dr][c+dc];
//          if (cell != '.') box.push_back(cell);
//        }
//      }
//      set<int> s(box.begin(), box.end());
//      if (box.size() != s.size()) return false;
//    }
//  }
//  return true;
//}
...
//    for (int c = 0; c < 9; c++) {
//      if (modified[r][c] - '0' == n) {
//        for (int i = 0; i < 9; i++) if (modified[r][i] == '.') modified[r][i] = 'x';
//        for (int i = 0; i < 9; i++) if (modified[i][c] == '.') modified[i][c] = 'x';
//        for (int dr = 0; dr < 3; dr++) 
//          for (int dc = 0; dc < 3; dc++) {
//            int br = 3 * (r / 3) + dr;
//            int bc = 3 * (c / 3) + dc;
//            if (modified[br][bc] == '.') modified[br][bc] = 'x';
//          }
//
//      }
//    }
...
//
//bool solve() {
//  bool ok = ( check_rows() and check_cols() and check_box() );
//  if (!ok) return false;
//
//  bool more = true;
//  while (more) {
//    more = false;
//    for (int n = 1; n < 10; n++) {
//      for (int i = 0 ; i < 9; i++) for (int j = 0 ; j < 9; j++) modified[i][j] = grid[i][j];
//        
//      cross(n);
//      int result = hatch(n);
//      if (result = 1) more = true;
